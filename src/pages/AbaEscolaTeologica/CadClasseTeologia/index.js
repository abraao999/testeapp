...
export default function CadClasseTeologia({ match }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (!id) {
        const response = await axios.post('/teologiaClasse', {
          descricao,
          setor_id: dataStorage.user.setor_id,
        });
        const novaLista = await axios.get('/teologiaClasse');
        setDescricaoList(novaLista.data);
        setDescricao('');
        toast.success('Classe criada com sucesso');
        setIsLoading(false);
      } else {
        const response = await axios.put(`/teologiaClasse/${id}`, {
          descricao,
        });
        const novaLista = await axios.get('/teologiaClasse');
        setDescricaoList(novaLista.data);
        ...
        history.push('/cadClasseTeologia');
        setIsLoading(false);
      }
    } catch (error) {
